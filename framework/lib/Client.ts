...
export class NekoChanClient extends Client {
     ...
     */
    public async initaliseClient(options: IClientOptions): Promise<void> {
        if (options.presence) {
            this.editStatus(options.presence.status, options.presence.activities);
        }

        if (this.debug) {
            const path = join(__dirname, "Events", "GEDebug.js");
            const { event } = await import(path);

            this.on("debug", event.run.bind(null, this));
        }
    ...
    public intialiseEvents(): void {
        ...
        readdirSync(eventPath).filter((file) => file !== "GEDebug.js" && file.startsWith("GE") && file.endsWith(".js")).forEach(async (file) => {
