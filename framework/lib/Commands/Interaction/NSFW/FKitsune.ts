 ...
 */
export async function kitsuneINSFWInteraction(client: NekoChanClient, interaction: CommandInteraction<TextableChannel> | ComponentInteraction<TextableChannel>, panelMode = false) {
    const response = await (new ImagesNSFW(client.fluxpoint).getKitsune());
